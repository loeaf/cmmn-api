package com.spo.cmmn.admin.stats.orgUsePerf.controller;
...
@Controller
@RequestMapping(value="/api/admin/sys/orgUsePerf")
public class OrgUsePerfController {
	private final Logger LOGGER = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());
	
	
	@Autowired
	OrgUsePerfService orgUsePerfService;
	...
	public ResponseEntity<List<?>> selectList(@RequestParam HashMap<String, Object> params, HttpServletRequest request) throws Exception {
		...
		List<?> gList = orgUsePerfService.selectList(params, request);
